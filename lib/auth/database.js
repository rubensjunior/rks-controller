...
class DatabaseProvider extends AuthProvider {
    ...
    async getIdentity(username) {
        ...
        let results = await this.db
            .select(this.users.identity, this.users.username)
    ...
    async getUsername(identity) {
        if (!identity) return null;

        let results = await this.db
            .select(this.users.username)
            .from(this.users.table)
            .where(this.users.identity, identity);
        
        for (let result of results) {
            if (result[this.users.identity] == identity) {
                return result[this.users.username];
            }
        }
