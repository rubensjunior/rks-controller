...
class AuthProvider {
    ...
    constructor(app, opts, name) {
        ...
        this.username = this.app.getSession(this.name + 'Username') || null;
    ...
    async autoLogin() {
        ...
        if (this.basicAuth) {
            ...
            if (auth) {
                await this.login(auth.username, auth.password, false, true);
                // If basic auth succeeded, don't try cookie login
        ...
        // Try cookie login only if basic auth didn't succeed
    ...
    async impersonate(identity, username) {
        if (!identity && !username) {
            throw new Error('auth.impersonate: identity or username is required.');
        }

        if (!identity) identity = await this.getIdentity(username);
        if (!username) username = await this.getUsername(identity);

        if (!username || !identity) {
            throw new Error('Cannot impersonate unknown identity: ' + (identity || username));
        }

        this.app.setSession(this.name + 'Id', identity);
        this.app.setSession(this.name + 'Username', username);
        this.app.set('identity', identity);
        this.app.set('username', username);
