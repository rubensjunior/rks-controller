...
// Threshold for using worker thread (100 rows x 100 rows = 10,000 comparisons)
const WORKER_JOIN_THRESHOLD = 100;
...
module.exports = {
    ...
    join: async function(options) {
        ...
        // Use worker thread for large joins to prevent blocking (O(n*m) complexity)
        if (collection1.length > WORKER_JOIN_THRESHOLD && collection2.length > WORKER_JOIN_THRESHOLD) {
            const workerFile = pathJoin(__dirname, '..', 'workers', 'collections-join.js');
            return runWorker(workerFile, { collection1, collection2, matches, matchAll });
        }

        // For small collections, join inline
        let output = [];
