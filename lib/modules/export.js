...
// Threshold for using worker thread (1000 rows or estimated 100KB)
...
module.exports = {
    ...
    csv: async function(options) {
        ...
        // Use worker thread for large datasets to prevent blocking
        if (data.length > WORKER_ROW_THRESHOLD) {
            const workerFile = join(__dirname, '..', 'workers', 'csv-exporter.js');
            const csvContent = await runWorker(workerFile, { data, delimiter, header });
            await fs.writeFile(toSystemPath(path), csvContent, { flag: overwrite ? 'w' : 'wx' });
            return path;
        }

        // For small datasets, export inline
