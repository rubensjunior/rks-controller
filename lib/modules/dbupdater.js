module.exports = {
    ...
    insert: async function (options) {
        ...
        if (options.test) {
            return {
                options: options,
                query: db.fromJSON(sql).toSQL().toNative()
            };
        }
            ...
            return db.transaction(async trx => {
                ...
                if (!Array.isArray(updated)) {
                    ...
                    const single = (
                        ...
                        (sql.wheres.rules[0].data ? sql.wheres.rules[0].data.column : sql.wheres.rules[0].column) == sql.returning &&
    ...
    delete: async function (options) {
        ...
        if (sql.sub) {
            return db.transaction(async trx => {
                ...
                // First, get the IDs that will be deleted (before actually deleting)
                // We need to do this BEFORE the DELETE because we need the IDs for cascade
                const idsToDelete = await trx.fromJSON({
                    ...sql,
                    type: 'select',
                    columns: [sql.returning]
                });

                deleted = idsToDelete.map(value => value[sql.returning]);

                // Now perform the actual delete
                await trx.fromJSON(sql);

                // loop sub - cascade delete related data
