...
module.exports = {
            ...
            if (Object.hasOwn(repeater, key)) {
                ...
                try {
                    ...
                    if (error instanceof BreakError) {
                        this.error = false; // Clear error so it doesn't propagate to parent loops
    ...
    exec: async function(options) {
        ...
        if (this.error instanceof BreakError) {
            this.error = false; // Clear the break error so it doesn't propagate further
        }
    ...
    sse: function(options) {
        let eventData = this.parseRequired(options.eventData, '*', 'core.emitevent: eventData is required.');
        let eventType = this.parseOptional(options.eventType, 'string', 'message');

        if (this.res.emitSSE) {
            this.res.emitSSE(eventData, eventType);
        }
    },
