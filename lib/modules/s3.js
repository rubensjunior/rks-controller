...
module.exports = {
    ...
    createFolder: async function (options) {
        const provider = this.parseRequired(options.provider, 'string', 's3.createFolder: provider is required.');
        const Bucket = this.parseRequired(options.bucket, 'string', 's3.createFolder: bucket is required.');
        const Key = this.parseRequired(options.key, 'string', 's3.createFolder: key is required.');
        const ACL = this.parseOptional(options.acl, 'string', undefined);
        const s3 = this.getS3Provider(provider);

        if (!s3) throw new Error(`S3 provider "${provider}" doesn't exist.`);

        // S3 doesn't have a specific folder creation method, we create a zero-byte object with a trailing slash
        return s3.putObject({ Bucket, Key: Key.endsWith('/') ? Key : `${Key}/`, ACL });
    },
    ...
    putFile: async function (options) {
        ...
        const ACL = this.parseOptional(options.acl, 'string', undefined);
        const Metadata = this.parseOptional(options.metadata, 'object', undefined);
        const s3 = this.getS3Provider(provider);
        ...
        const result = await s3.putObject({ Bucket, ACL, Key, ContentType, ContentDisposition, Body, Metadata });
    ...
    signUploadUrl: async function (options) {
        ...
        const command = new PutObjectCommand({ Bucket, Key, ContentType, ACL, Metadata });
