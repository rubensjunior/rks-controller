...
hotp.generate = function (secret, opts) {
  ...
  const hash = crypto.createHmac(opts.algorithm, secret).update(buffer).digest();
...
module.exports = {
  ...
  hotpValidate (options) {
    ...
    const counter = this.parseOptional(options.counter, 'number', null);
    const window = this.parseOptional(options.window, 'number', null);
    const digits = this.parseOptional(options.digits, 'number', 6); // 6 | 8
    const algorithm = this.parseOptional(options.algorithm, 'string', 'sha1'); // 'sha1' | 'sha224' | 'sha256' | 'sha384' | 'sha512' | 'sha3-224' | 'sha3-256' | 'sha3-384' | 'sha3-512'

    return hotp.validate(code, secret, { counter, window, digits, algorithm });
  },

  totpGenerate (options) {
    const secret = this.parseRequired(options.secret, 'string', 'otp.totpGenerate: secret is required.');
    let time = this.parseOptional(options.time, 'string', null);
  ...
  generateOtpUri (options) {
    ...
    return `otpauth://totp/${encodeURIComponent(issuer)}:${encodeURIComponent(account)}?secret=${secret}&issuer=${encodeURIComponent(issuer)}&algorithm=${encodeURIComponent(algorithm.toUpperCase())}&digits=${digits}&period=${period}`;
  },
