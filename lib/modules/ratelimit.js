module.exports = {
   ...
   * @param {string} options.name - The name of the rate limiter instance (required)
   ...
   * @param {string} name - The step name (not used for limiter name)
   */
  setup: function (options, name) {
    // Extract name from options.name (required parameter)
    const limiterName = options.name;
    if (!limiterName) throw new Error('rateLimit.setup has no name.');

    // Remove name from options before passing to setRateLimiter
    const { name: _, ...limiterOptions } = options;

    this.setRateLimiter(limiterName, limiterOptions);
  },
  ...
  consume: function (options) {
    ...
    if (options.instance) {
      ...
      return this.req.app[isPrivate ? 'privateRateLimiter' : 'rateLimiter'].consume(key, options.points).then((rateLimiterRes) => {
