...
App.prototype = {
  ...
  withScope: async function(data, fn) {
    const parentScope = this.scope;
    this.scope = this.scope.create(data);
    try {
        await fn();
    } finally {
  ...
  setAuthProvider: async function (name, options) {
    ...
    const providerType = (options.provider || options.type || '').toLowerCase();
    if (!providerType) {
      throw new Error('auth.provider: provider type is required.');
    }
    const Provider = require('../auth/' + providerType);
  ...
  setDbConnection: function (name, options) {
    ...
    const formatRecord = (record, meta) => {
      for (column in record) {
        ...
        // Convert undefined to null for all columns (moved outside the != null check)
        if (record[column] == undefined) {
          record[column] = null;
        } else if (record[column] != null && record[column].toJSON) {
  ...
  setJSONWebToken: function (name, options) {
    ...
    if (options.alg || options.algorithm) opts.algorithm = options.alg || options.algorithm;
    ...
    // Support both Server Connect names (claims/key) and standard JWT names (payload/secret)
    const payload = options.claims || options.payload || {};
    const secretOrKey = options.key || options.secret;

    if (!secretOrKey) {
      throw new Error('jwt.sign: key or secret is required.');
    }
  ...
  _exec: async function (steps, ignoreAbort) {
    ...
    if (steps.action) {
      try {
        ...
        if (fs.existsSync(path.resolve(__dirname, `../../extensions/server_connect/modules/${steps.module}.js`))) {
          module = require(`../../extensions/server_connect/modules/${steps.module}`);
        } else if (fs.existsSync(path.resolve(__dirname, `../modules/${steps.module}.js`))) {
        ...
        if (steps.name) {
          ...
          if (steps.output || config.debugOutputAll) {
